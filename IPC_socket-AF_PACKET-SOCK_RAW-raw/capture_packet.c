...
static int ethdump_initSocket() {
    ...
    /* 设置SOCKET选项 设置接受缓冲区 size 为g_iRecvBufSize */
    ...
    iRet = bind(fd, (struct sockaddr *)&stLocal, sizeof(stLocal)); //绑定本地网卡 eth0
    if (0 > iRet) {
...
static int ethdump_parseEthHead(const struct ether_header *pstEthHead) {
    ...
    //printf("IN ethdump_parseEthHead \n");
...
static int ethdump_parseIpHead(const struct ip *pstIpHead) {
    ...
    //printf("IN ethdump_parseIpHead \n");
    
    /* 协议类型、源IP地址、目的IP地址 */
    pstIpProto = getprotobynumber(pstIpHead->ip_p);
    //perror("IN ethdump_parseIpHead 2 \n");
    if(NULL != pstIpProto) {
...
static int ethdump_parseFrame(const char *pcFrameData) {
    ...
    //printf("IN ethdump_parseFrame \n");
    /* Ethnet帧头解析 */
    pstEthHead = (struct ether_header*)g_acRecvBuf;
    iRet = ethdump_parseEthHead(pstEthHead);
    if (0 > iRet) {
        printf("ethdump_parseFrame error \n");
        return iRet;
    }
    
    /* IP数据包类型 */
    pstIpHead  = (struct ip *)(pstEthHead + 1);  // + 1表示跳过 sizeof(struct ether_header)个字节，到达ip头部
